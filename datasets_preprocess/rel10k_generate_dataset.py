...
# from scipy.misc import imresize  # Remove this import
...
def process(data, seq_id, videoname, output_root):
    ...
    for pngname in png_list:
        ...
        # Replace imresize with Pillow's resize
        pil_image = Image.open(pngname)
        pil_image = pil_image.resize((int(pil_image.size[0]/2), int(pil_image.size[1]/2)), Image.BILINEAR)
        pil_image.save(pngname)  # Save the resized image
        
        # In your case, the commented-out code with OpenCV is not necessary anymore
...
if __name__ == "__main__":
    ...
    dataroot = "/ssd2/wenyan/3r_benchmark/preprocess/RealEstate10K_test_remaining/" + mode
