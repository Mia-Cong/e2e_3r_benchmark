...
def save_sample_data(sample, dest_root, source_root):
    ...
    if "images" in sample.data:
        for idx, image_obj in enumerate(sample.data["images"]):
            src_image_path = os.path.join(source_root, image_obj.path)
    ...
    if "depth" in sample.data:
        ...
        src_depth_path = os.path.join(source_root, depth_obj.path)
...
# Example usage
pickle_file_path = "/ssd2/wenyan/3r_benchmark/robustmvd/rmvd/data/sample_lists/kitti.robustmvd.mvd.pickle"
dest_root = "/ssd2/wenyan/3r_benchmark/datasets/RobustMVD_mvdsplit/kitti"
source_root = "/ssd2/wenyan/3r_benchmark/datasets/RobustMVD/kitti"  # Root directory of the source dataset
