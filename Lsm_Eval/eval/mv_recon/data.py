...
class TUM(BaseStereoViewDataset):
    def __init__(
        self,
        num_seq=1,
        num_frames=5,
    ...
    def load_poses(self, gt_traj_file):
        ...
        #assert False, [xyz.shape, quat.shape, timestamps_mat.shape]
        N = xyz.shape[0]
        poses = []
        for i in range(N):
            ...
            w2c_pose = np.eye(4, dtype=np.float32)
            w2c_pose[:3, :3] = rotation
            w2c_pose[:3, 3] = [tx, ty, tz]
            
            c2w_pose = np.linalg.inv(w2c_pose)
            # Assign the appropriate pose (use c2w_pose if dust3r expects c2w)
            poses.append(w2c_pose)
            #poses.append(c2w_pose)
        
        return np.array(poses, dtype=np.float32), [True for _ in range(N)] 
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        if self.tuple_list is not None:
            ...
            num_files = len(os.listdir(os.path.join(self.ROOT, scene_id, "rgb_12k_stride30")))
            img_idxs = [f"{i}" for i in range(num_files)]
            img_idxs = img_idxs[:: min(self.kf_every, len(img_idxs) // 2)]
        ...
        posepath = osp.join(self.ROOT, scene_id, f"groundtruth_12k_stride30.txt")
        camera_poses, valids = self.load_poses(posepath)
        ...
        impaths = sorted(os.listdir(osp.join(self.ROOT, scene_id, "rgb_12k_stride30")))
        impaths = [osp.join(self.ROOT, scene_id, "rgb_12k_stride30", f) for f in impaths]
        depthpaths = sorted(os.listdir(osp.join(self.ROOT, scene_id, "depth_12k_stride30")))
        depthpaths = [osp.join(self.ROOT, scene_id, "depth_12k_stride30", f) for f in depthpaths]

        while len(imgs_idxs) > 0:
            im_idx = imgs_idxs.popleft()

            impath = impaths[int(im_idx)]
            depthpath = depthpaths[int(im_idx)]

            rgb_image = imread_cv2(impath)
            
            # loads depth map D from png file
            # and returns it as a numpy array
            depth_png = np.asarray(Image.open(depthpath))
            # make sure we have a proper 16bit depth map here.. not 8bit!
            #assert np.max(depth_png) > 255
            depthmap = depth_png.astype(np.float64) / 5000.0
            depthmap[depth_png == 0] = 0.0 # this is different from the depth eval tasks!
            depthmap = depthmap.astype(np.float32) # this is different from the depth eval tasks!
            ...
            # gl to cv
            #camera_pose[:, 1:3] *= -1.0
            if True or resolution != (224, 224) or self.rebuttal:
                rgb_image, depthmap, intrinsics = self._crop_resize_if_necessary(
                    rgb_image, depthmap, intrinsics_, resolution, rng=rng, info=impath
                )
            else:
...
class SCANNET(BaseStereoViewDataset):
    ...
    def load_all_scenes(self, base_dir):
        ...
        scenes = [
            d for d in os.listdir(base_dir) if os.path.isdir(os.path.join(base_dir, d)) and d not in bad_scenes
        ]
        if self.test_id is not None:
            self.scene_list = [self.test_id]
    ...
    def load_poses(self, path):
        traj_w_c = np.loadtxt(path)
        assert traj_w_c.shape[1] == 16
        w2cs = traj_w_c.reshape(-1, 4, 4)
        
        #poses = []
        #for w2c in w2cs:
        #    poses.append(np.linalg.inv(w2c))
        poses = w2cs
        return np.array(poses, dtype=np.float32), [True for _ in poses]
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        if self.tuple_list is not None:
            ...
            num_files = len(os.listdir(os.path.join(self.ROOT, scene_id, "color_12k_stride30")))
            img_idxs = [f"{i}" for i in range(num_files)]
            img_idxs = img_idxs[:: min(self.kf_every, len(img_idxs) // 2)]

        
        fx, fy, cx, cy = 575.665710, 578.051880, 320.287018, 240.126617
        intrinsics_ = np.array([[fx, 0, cx], [0, fy, cy], [0, 0, 1]], dtype=np.float32)

        posepath = osp.join(self.ROOT, scene_id, f"pose_12k_stride30.txt")
        camera_poses, valids = self.load_poses(posepath)
        ...
        impaths = sorted(os.listdir(osp.join(self.ROOT, scene_id, "color_12k_stride30")))
        impaths = [osp.join(self.ROOT, scene_id, "color_12k_stride30", f) for f in impaths]
        depthpaths = sorted(os.listdir(osp.join(self.ROOT, scene_id, "depth_12k_stride30")))
        depthpaths = [osp.join(self.ROOT, scene_id, "depth_12k_stride30", f) for f in depthpaths]
